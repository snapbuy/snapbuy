set.seed(003) # 나중에 재현을 위한 seed값 설정

#데이터불러올 경로 설정 
setwd("C:/Users/leeyua/Desktop/에이림_R")
getwd()
lol<- read.csv("lol_mvp.csv",header=TRUE)
# 데이터에 대한 자세한 설명
str(lol)
...
library(reshape2) 
#결측값을 0값으로 바꾸기 / 결측값을 평균값으로 바꾸기
example_data[is.na(example_data)] <-0
example_data[is.na(example_data)]<-mean(example_data$c_thickness,na.rm = TRUE)
#첫번째,아홉번째 열 삭제 
